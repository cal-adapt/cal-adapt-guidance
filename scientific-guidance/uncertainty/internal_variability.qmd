...
Internal variability is an irreducible component of climate uncertainty — it cannot be narrowed by improving models or constraining emissions. The appropriate response is to sample it explicitly, through multiple ensemble members, and to communicate the resulting range honestly. For a deeper scientific treatment, see the IPCC AR5 uncertainty guidance [@ipcc-uncertainty-guidance-2010] and Hawkins & Sutton [@hawkinsPotential2009].
:::
...
```
![](figures/static/members_variability.png){.d-md-none fig-alt="Grid of maps showing change in 99th-percentile monthly precipitation per CMIP6 ensemble member, organized by model (columns) and ensemble member (rows)."}
:::
::: {.content-visible when-format="pdf"}
![](figures/static/members_variability.png){fig-alt="Grid of maps showing change in 99th-percentile monthly precipitation per CMIP6 ensemble member, organized by model (columns) and ensemble member (rows)."}
:::
...
```
![](figures/static/internal_var_vs_model_spread.png){.d-md-none fig-alt="Range plot comparing internal variability (per-model ensemble spread) to inter-model spread in 99th-percentile monthly precipitation change, with each model's downscaled WRF member marked."}
:::
::: {.content-visible when-format="pdf"}
![](figures/static/internal_var_vs_model_spread.png){fig-alt="Range plot comparing internal variability (per-model ensemble spread) to inter-model spread in 99th-percentile monthly precipitation change, with each model's downscaled WRF member marked."}
:::
...
```
![](figures/static/warming_ridgeplots.png){.d-md-none fig-alt="Ridgeline plot of kernel density estimates showing each model's within-ensemble distribution of 99th-percentile monthly precipitation change, with the single downscaled WRF member marked per model."}
:::
::: {.content-visible when-format="pdf"}
![](figures/static/warming_ridgeplots.png){fig-alt="Ridgeline plot of kernel density estimates showing each model's within-ensemble distribution of 99th-percentile monthly precipitation change, with the single downscaled WRF member marked per model."}
:::
...
```
![](figures/static/ks_99_precip_no_sig.png){.d-md-none fig-alt="Map of change in 99th-percentile monthly precipitation computed from the multi-model mean."}

![](figures/static/ks_99_precip_sig.png){.d-md-none fig-alt="Map of change in 99th-percentile monthly precipitation computed from the pooled model-month sample, with stippling marking grid cells where a Kolmogorov–Smirnov test finds a statistically significant difference."}
:::
::: {.content-visible when-format="pdf"}
![](figures/static/ks_99_precip_no_sig.png){fig-alt="Map of change in 99th-percentile monthly precipitation computed from the multi-model mean."}

![](figures/static/ks_99_precip_sig.png){fig-alt="Map of change in 99th-percentile monthly precipitation computed from the pooled model-month sample, with stippling marking grid cells where a Kolmogorov–Smirnov test finds a statistically significant difference."}
:::
