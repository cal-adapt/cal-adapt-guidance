...
![Grid of maps showing change in 99th-percentile monthly precipitation per CMIP6 ensemble member, organized by model (columns) and ensemble member (rows).](figures/static/members_variability.png)
:::
...
:::
::: {.content-visible when-format="pdf"}
![Range plot comparing internal variability (per-model ensemble spread) to inter-model spread in 99th-percentile monthly precipitation change, with each model's downscaled WRF member marked.](figures/static/internal_var_vs_model_spread.png)
:::
...
:::
::: {.content-visible when-format="pdf"}
![Ridgeline plot of kernel density estimates showing each model's within-ensemble distribution of 99th-percentile monthly precipitation change, with the single downscaled WRF member marked per model.](figures/static/warming_ridgeplots.png)
:::
...
:::
::: {.content-visible when-format="pdf"}
![Map of change in 99th-percentile monthly precipitation computed from the multi-model mean.](figures/static/ks_99_precip_no_sig.png)

![Map of change in 99th-percentile monthly precipitation computed from the pooled model-month sample, with stippling marking grid cells where a Kolmogorov–Smirnov test finds a statistically significant difference.](figures/static/ks_99_precip_sig.png)
:::
