...
The lookup tables described here are used for selecting data from each model, but are not an accurate way to estimate what year a GWL is most likely to be reached. To estimate the timing of GWLs, the Analytics Engine uses the IPCC's warming trajectories, as described on the [Climate Model Data](/data-tools/climate-model-data.qmd) page.
::: 
